#!/usr/bin/env bash
# list-startable.sh — thin shim. The transition logic now lives in kraken.py (one
...
#
# usage: list-startable.sh OWNER/tasks PROJECT [--snapshot]
exec python3 "$(dirname "$0")/kraken.py" list-startable "$@"
