#!/usr/bin/env bash
# escalate.sh — thin shim. The transition logic now lives in kraken.py (one
...
#
# usage: escalate.sh OWNER/tasks ISSUE WORKER_NAME QUESTION_FILE
exec python3 "$(dirname "$0")/kraken.py" escalate "$@"
