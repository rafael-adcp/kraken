#!/usr/bin/env bash
# deliver.sh — thin shim. The transition logic now lives in kraken.py (one
...
#
# usage: deliver.sh OWNER/tasks ISSUE WORKER_NAME RESULT_FILE [PR_URL]
exec python3 "$(dirname "$0")/kraken.py" deliver "$@"
