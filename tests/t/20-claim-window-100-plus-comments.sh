...
mk_comment 50 '<!-- kraken {"type":"claim","worker":"dead-worker"} -->'
...
mk_comment 50 '<!-- kraken {"type":"stale-claim","reason":"no activity for 7h"} -->'
# comment #102 (past the boundary): a second worker wins the fresh window.
mk_comment 50 '<!-- kraken {"type":"claim","worker":"heir"} -->'

# A third worker now tries to claim it. If arbitration only saw the first 100
# comments, it would see #1's dead-worker claim with no reset in view
...
# the reset clears dead-worker and #102's heir claim is the true winner.
