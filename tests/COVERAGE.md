...
| §2.1 validation is a read | A console **SHOULD** report every open task missing a `project:<name>` label, Goal or Acceptance; a project-scoped report **MUST** still surface a task with no project label; it **MUST NOT** block, close or relabel, and a worker **MUST NOT** refuse a task on these grounds | ✅ pinned | `tests/unit/test_kraken.py` `QueueHygieneTests` pins the three checks, the `_No response_` rule and the scope rule (a project-less task is reported under any scope, another project's is not); `tests/conformance/test_23_status.py` `test_queue_hygiene_reports_what_a_worker_cannot_start` drives it end to end and proves a compliant queue reports nothing. The optional comment form stays an operator-invoked subcommand, pinned by `tests/conformance/test_25_validate_task.py` (missing label, missing Acceptance, heading-less body, compliant, non-task, debounce, edit-after-fix) and the section-parse/debounce units. |
...
| Clause (line) | Normative text | Status | Pinned by |
| --- | --- | --- | --- |
| §6 renewal | A worker holding a lease **SHOULD** renew it every TTL/3, by creating the next generation; a refused renewal (422) means it was stolen from and it **MUST** stop | ✅ pinned (cadence 🧠 agent) | Cadence is worker behavior, but it is no longer prose the model has to remember: `next-action`'s envelope carries `renew_every_seconds`, `expires_at` and `renew_now`, and `then.renew` is the exact command (`tests/conformance/test_39_next_action.py`, `NextActionEnvelopeTests`). The renewal *mechanism* — climb a generation with a fresh commit, restart the TTL, collect the rung left behind, post no comment, do not free the lease, refuse when the lease is no longer ours — is pinned by `tests/conformance/test_08_heartbeat.py`, including that a renewal makes a lease one second from expiry unstealable again, and by `tests/conformance/test_11_write_transition_failures.py` (a failed create leaves the lease exactly where it was). That a renewal and a steal cannot both win: `tests/conformance/test_36_lease_steal_race.py`. |
| §6 hooks are optimization | Releasing on the way out is an **optimization**; nothing normative depends on a lifecycle hook or a supervising loop | ✅ pinned | Structural: recovery is pinned end to end with no hook in the picture by `tests/conformance/test_34_claim_next_reconcile.py::test_an_expired_lease_is_stolen_not_escalated` and `tests/conformance/test_35_lease_expiry.py`, which drive only `kraken.py`. The hooks' own fast path stays pinned by `tests/conformance/test_16_session_end_release.py`, `test_27_stop_failure_release.py` and `test_32_loop_fast_release.py`. |
| §6 the reader reconciles | A worker **MUST** reconcile before it claims; a conforming repo runs **no** scheduled job and a worker **MUST NOT** assume one | ✅ pinned | `tests/conformance/test_34_claim_next_reconcile.py` drives all four rules from `claim-next` alone — never `reap` — and proves the drain then claims out of the state it just repaired in the same invocation; it also pins the cost claim that makes this viable (flat call count from 1 to 20 live claims; no commit-date read at all when no lock is live) and that an already-agreeing queue costs zero writes. |
| §6 reconcile rules | Anchored to the claim ref's commit date (not `updatedAt`; nothing on the timeline resets it); an expired lease is **not** the reconciler's business; the four rules — delete orphan lock, reclaim a repeatedly expired task, heal a **live** lease's missing label, requeue an orphan projection; ref delete last | ✅ pinned | `tests/unit/test_kraken.py` `ReconcilerPlanTests` pins the rules as a pure decision — including that a first expiry (and an unreadable one) plans **nothing**, that heal never re-holds an expired lease, and that an agreeing queue plans nothing — and `ReconcilerApplyTests` pins the write ordering, the §4 attribution on the reclaim comment, and the in-memory fold the drain classifies from; `tests/conformance/test_17_reclaim_stale.py` drives the same pass stand-alone through `kraken.py reap`, proving an expired lease costs it zero writes; `tests/unit/test_workflow_commands.py` `ReapCommandTests` pins the wiring, the `--ttl` flag reaching the read, and the failure staging. |
| §6 repeat-expiry guard | A task whose lease has expired **N** times (reference: 3, counted from `lease-expired` markers) goes to `needs-decision` instead of being stolen again; a **live** lease is never reclaimed by the count | ✅ pinned | `tests/conformance/test_38_lease_expiry_escalation.py`: below the threshold the task is still stolen, at the threshold it reaches the operator with a `stale-claim` comment naming the count, a live lease is never evicted however long the history, and each steal leaves exactly one countable marker. Pure-decision side: `tests/unit/test_kraken.py` `ExpiryCountTests` + `ReconcilerPlanTests`. |
| §6 requeue by derivation | A reader **MUST** treat a label-held task as queued when an operator comment is newer than the newest worker comment; the asymmetry (bare comment requeues `needs-decision`, not `awaiting-merge`); a prose `requeue:` **MUST NOT** bounce a ready branch; a **live** lease outranks the thread, an expired one outranks nothing | ✅ pinned | `tests/conformance/test_18_requeue_by_derivation.py` asserts on what the QUEUE says (`list-startable`) and on the task actually being claimable — never on a label having been rewritten: bare reply, re-escalation re-holding, kraken-authored comments never requeueing (what retires the protocol/4 Bot gate), the awaiting-merge asymmetry, ref-outranks-thread, dependencies still applying, and `status` dropping an answered task from the decision queue. `tests/unit/test_workflow_commands.py` `RequeueVerdictTests` / `RequeuedLabelsTests` pin the verdict as a pure function (including that a truncated comment window cannot mislead), plus the human-vs-worker discrimination and requeue-directive detection. |
...
| Clause (line) | Normative text | Status | Pinned by |
| --- | --- | --- | --- |
| §10 closed labels are inert | A closed task's labels decide nothing — every read walks OPEN issues only — so cleanup is optional; a leftover claim ref is reclaimed by §6 rule 1, not by the close | ✅ pinned | Inertness is structural and pinned by the reads themselves: `fetch_open_tasks` filters `states: OPEN`, and `tests/conformance/test_34_claim_next_reconcile.py` proves a ref on a CLOSED issue is dropped as an orphan lock by the reconcile. The optional label strip stays an operator-invoked subcommand: `tests/conformance/test_21_cleanup_closed.py` plus `tests/unit/test_workflow_commands.py` (`IdentityLabelTests`, `CleanupCommandTests`) pin the keep/strip rule, the claim-ref delete, and the no-op/transport paths. |
