...
| Clause (line) | Normative text | Status | Pinned by |
| --- | --- | --- | --- |
| §5.1 lease TTL | The lease TTL is a duration in **minutes**, configurable, and the renewal interval is **derived** from it (TTL/3) | ✅ pinned | `tests/unit/test_kraken.py` `LeaseTtlTests` pins the resolution order (explicit → `KRAKEN_LEASE_TTL_SECONDS` → default), that a garbage or non-positive override falls back rather than expiring every live lease, that the default stays in minutes, and that renewal is TTL/3 with room for two lost renewals; `ContractCommandTests.test_lease_clock_is_readable_off_the_contract` pins `contract lease-ttl`/`lease-renew` as the single source SKILL.md and the docs quote. |
