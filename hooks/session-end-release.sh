...
# ~6h for the reaper. It reuses release.sh, so `released: <worker>` lands before
# in-progress drops — the ordering that closes the claim window (PROTOCOL.md §9).
#
...
#
# Discovery: claim.sh writes ${KRAKEN_STATE_DIR:-$HOME/.kraken}/claim-<worker>.json
...
# Locate the bundled scripts. CLAUDE_PLUGIN_ROOT is set when Claude Code runs
# the hook; the dirname fallback keeps the script runnable standalone (tests).
ROOT_DIR="${CLAUDE_PLUGIN_ROOT:-"$(cd "$(dirname "$0")/.." && pwd)"}"
RELEASE="$ROOT_DIR/skills/unleash/release.sh"
...
for f in "$STATE_DIR"/claim-*.json; do
  ...
  # Best-effort: release.sh posts released: then drops in-progress and removes
  # the state file. If it fails (network down), the reaper still backs us up —
  # do not let it fail the hook or block the session from exiting.
  bash "$RELEASE" "$repo" "$issue" "$worker" "session ended" >/dev/null 2>&1 || true
done
